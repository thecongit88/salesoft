...
/// Chi tiết khách hàng
///
class InventoryItemDetailQuotePricePage extends StatelessWidget {
  const InventoryItemDetailQuotePricePage({Key? key}) : super(key: key);
...
class _BackButtonWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return IconButton(
      ...
      onPressed: () {
        final InventoryItemDetailController controller = Get.find();
        Get.back(result: controller.isAddedBaoGia.value);
      },
    );
  }
}
                ...
                InkWellWidget(
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        Icon(Icons.add_circle, color: Colors.white, size: 14.sp,),
                        SizedBox(width: 5,),
                        Text(
                          "Chọn báo giá",
                  ...
                  onPress: () {
                      print("Chọn báo giá");
                      var item = new InventroyItemModel(
                          code: productDetail!.code,
                          name: productDetail!.name,
                          Price: productDetail?.giaBaoGia ?? 0 //price là giá lẻ - giá báo cho khách
                      );
                      item.soluongBaoGia = productDetail!.soluongBaoGia + 1;
                      productDetail!.soluongBaoGia = item.soluongBaoGia;
                      var controllerCart = Get.find<InventoryItemQuotePriceController>();
                      controllerCart.calcQuoteCart4Item(item);

                      final InventoryItemDetailController controller = Get.find();
                      controller.isAddedBaoGia.value = true;
                  },
                )
              ],
            ),
